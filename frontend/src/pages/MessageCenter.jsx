...
export default function MessageCenter({ onBack }) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        // 获取所有待处理的通知
        const [reqRes, propRes] = await Promise.allSettled([
  ...
  const handleRequestAction = async (requestId, action, teamName, request) => {
    try {
      ...
      alert(`${action === 'APPROVED' ? (request.type === 'INVITE' ? '接受' : '批准') : '拒绝'}成功`);
    } catch (err) {
      console.error('处理申请失败:', err);
      alert('处理失败: ' + (err.response?.data?.error || err.message));
    }
  };

  const handleProposalAction = async (proposalId, matchId, action) => {
    try {
      // 选择第一个提议时间（简化处理，实际用户选择后这里应该传入选定时间）
      // 这里只做 accept/reject，实际选定时间由前端选择后传入
      await api.put(`/match-proposals/${proposalId}/respond`, {
        action: action === 'ACCEPTED' ? 'ACCEPT' : 'REJECT',
        selectedTime: null // 简化处理，实际可以让用户选择
      });
      // 刷新列表
      setPendingProposals(prev => prev.filter(p => p.proposalId !== proposalId));
      alert(`${action === 'ACCEPTED' ? '接受' : '拒绝'}成功`);
    } catch (err) {
  ...
  const totalUnread = pendingRequests.length + pendingProposals.length;
                ...
                <div key={prop.proposalId} className="border-b border-gray-200 py-6 px-2 hover:bg-gray-50 transition-colors">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    {!prop.isInitiator && (
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleProposalAction(prop.proposalId, prop.matchId, 'ACCEPTED')}
                          className="bg-black text-white px-4 py-2 text-xs font-bold hover:bg-[#660874] transition-colors"
                        >
                          接受
                        </button>
                        <button
                          onClick={() => handleProposalAction(prop.proposalId, prop.matchId, 'REJECTED')}
                          className="border border-red-500 text-red-500 px-4 py-2 text-xs font-bold hover:bg-red-500 hover:text-white transition-colors"
                        >
                          拒绝
                        </button>
